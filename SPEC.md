...
26. **RabbitMQ reconnect = in-service retry loop with consumer replay; Kafka left to kafkajs** (2026-07-11, post-review hardening). `amqplib` does not reconnect on its own — a dropped connection left consumers as silent zombies until process restart. Both RabbitMQ wrappers in `shared-messaging` now listen for connection/channel close, tear down, and retry every 5s forever; the consumer replays every registered subscription on the fresh channel, the publisher fails fast mid-reconnect (the outbox row is the buffer — decision 7's relay retries, no in-memory buffering). Initial connect stays fail-fast at boot (broker down = boot failure, not half-alive service). Kafka needs no equivalent: kafkajs retries/reconnects internally and restarts retriable consumer crashes; non-retriable crash stays a manual-restart concern (documented in `kafka-consumer.service.ts`), now made loud via a `CRASH`-event error log.
27. **Contract governance = frozen-fixture compatibility tests + consumer-side version check** (2026-07-11, post-review hardening). Decision 9's additive-only rule and decision 13's `eventVersion` field were honor-system — nothing failed CI on a breaking `shared-dtos` change and no consumer read the version. Now: frozen v1 wire payloads in `contract-compatibility.spec.ts` must forever validate against current DTO classes (backward) and tolerate unknown extra fields (forward), so a breaking change fails `nx test shared-dtos`; an `EVENT_VERSIONS` registry + `assertSupportedEventVersion()` runs at all four consumer boundaries — RabbitMQ consumers throw (retry → DLQ per BUG-0007), Kafka consumers log + skip (no Kafka DLQ; blocking the partition would be worse). Schema registry rejected as new infra for a one-team workspace. Full rationale: [ADR-0012](./docs/decisions/ADR-0012-contract-compatibility-enforcement.md).
28. **Observability first pass = edge-minted correlationId through the whole chain + ids in every messaging failure log + `LOG_FORMAT=json`** (2026-07-11, post-review hardening). Decision 8's correlationId was minted one hop too late (Content Service, not the gateway) and nothing ever read it. Now: gateway mints per request, forwards as `x-correlation-id`, returns it to the client; Content Service carries it into the outbox; both broker consumers log messageId + correlationId on every failure (Kafka via catch-log-rethrow, preserving retry semantics); all five services support Nest's built-in JSON console logger via `LOG_FORMAT=json`. Metrics endpoints + scrape stack and OTel tracing deferred — new infra, ask-first boundary. pino and AsyncLocalStorage context rejected as premature at this log volume. Full rationale: [ADR-0013](./docs/decisions/ADR-0013-observability-first-pass.md).
