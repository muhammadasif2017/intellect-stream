...
- **PROPOSED, pending engineer review** — **Integration test approach = real docker-compose infra, not testcontainers; own `integration` nx target, not swept into `test`** (2026-07-10, milestone 8). Picked autonomously (engineer unavailable, asked to keep going) — flagging rather than logging as accepted, since milestone 8 establishes the testing approach from scratch and that call belongs to the engineer, unlike decisions 19–22 where the engineer had already chosen and this log just recorded it. Rationale as selected: this repo has no `testcontainers` dependency and already runs a long-lived docker-compose stack in dev (Quick Start assumes it's up before `nx serve`); reusing that stack is less new machinery than adding ephemeral-container orchestration. New `apps/e2e-tests` project, `jest.integration.config.ts` (deliberately not named `jest.config.ts`, so `@nx/jest`'s plugin doesn't auto-infer a `test` target and sweep these into `nx run-many -t test`/CI, which has no live infra). Run via `nx run e2e-tests:integration`. First test: a real RabbitMQ publish/consume round-trip through `shared-messaging`'s actual classes, proving the environment can run tests against real infra before anything bigger was built on top. Cloudflare Workers AI (real third-party paid API, no configurable endpoint in `cf-workers-ai.service.ts`) is mocked at the HTTP layer even in these "real infra" tests — the one hop that can't reasonably run for real. Built on top of the probe: a second test walks the actual golden path (post → outbox → RabbitMQ → AI Processing (CF mocked) → RabbitMQ → Content Service updates + re-outboxes → Kafka → Analytics Service persists a trend row + Notification Service pushes to a registered socket), bootstrapping all four services' real `AppModule`s via `NestFactory.createApplicationContext` (cross-app imports, flagged inline with `eslint-disable` comments — reaching into another app's `src`, not a lib, is a real deviation from this workspace's Nx conventions, done only for test wiring). Both tests pass against the live docker-compose stack (`nx run e2e-tests:integration`, ~40s). One rough edge hit and fixed: without `--forceExit`, Jest hangs after the suite passes (open Kafka/RabbitMQ handles not fully released even after `app.close()`) — added `--forceExit` to the target. **Needs your explicit accept/reject; move to the Decisions Log (or replace) once you've reviewed it.**
